  ...
  if (authLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-emerald-50 to-white dark:from-gray-950 dark:to-gray-900">
        <div className="text-emerald-600 animate-pulse text-lg">로딩 중...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white dark:from-gray-950 dark:to-gray-900">
      ...
      <main className="max-w-4xl mx-auto px-4 pb-24">
        ...
        <div className="flex items-center gap-3 mb-6">
          {trees.length > 0 && (
            <select value={selectedTreeId} onChange={e => setSelectedTreeId(e.target.value)}
              className="px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm font-medium">
              {trees.map(t => (
          ...
          <button onClick={() => setShowCreateTree(true)}
            className="px-3 py-2 rounded-lg border border-dashed border-gray-300 dark:border-gray-700 text-gray-500 text-sm hover:border-emerald-500 hover:text-emerald-600 transition-colors">
            + 새 가계도
          </button>
        </div>
          ...
          <div className="mb-6 p-4 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
            ...
            <div className="flex gap-2">
              <input type="text" value={newTreeName} onChange={e => setNewTreeName(e.target.value)}
                className="flex-1 px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm"
                placeholder="예: 우리 가족, 김씨 가계도" autoFocus
                onKeyDown={e => e.key === 'Enter' && handleCreateTree()} />
              <button onClick={handleCreateTree}
                className="px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700">만들기</button>
              <button onClick={() => setShowCreateTree(false)}
                className="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 text-gray-500 text-sm">취소</button>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-1 mb-6 bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
          {([
            { key: 'tree' as Tab, label: '가계도', icon: '🌳' },
            { key: 'events' as Tab, label: '일정', icon: '📅' },
            { key: 'ledger' as Tab, label: '부조/선물', icon: '🎁' },
            { key: 'memories' as Tab, label: '추억', icon: '📝' },
          ]).map(t => (
        ...
        {loading ? (
          <div className="text-center py-20 text-emerald-600 animate-pulse">데이터 로딩 중...</div>
        ) : (
          <>
            {/* 가계도 탭 */}
            {tab === 'tree' && (
              <div className="space-y-6">
                ...
                {/* 가계도 뷰 */}
                <div className="p-4 rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 overflow-hidden">
    ...
    <header className="sticky top-0 z-30 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-800">
      <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Link href="/" className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 text-sm">YH Hub</Link>
...
function MemberInfoCard({ member, members, relations, memories, ledger, onClose }: {
  ...
  return (
    <div className="p-4 rounded-2xl bg-white dark:bg-gray-900 border border-emerald-200 dark:border-emerald-800 relative">
      <button onClick={onClose} className="absolute top-3 right-3 text-gray-400 hover:text-gray-600 text-lg">&times;</button>
        ...
        <div className="mt-4">
          ...
          <div className="space-y-2">
            {memberMemories.slice(0, 3).map(m => (
              <div key={m.id} className="p-2 rounded-lg bg-gray-50 dark:bg-gray-800">
...
function EventsTab({ events, members }: { events: FamilyEvent[]; members: FamilyMember[] }) {
  ...
  if (events.length === 0) {
    return <EmptyState icon="📅" message="등록된 일정이 없습니다" sub="일정을 추가해보세요" />;
  }

  return (
    <div className="space-y-3">
      {events.map(e => (
        <div key={e.id} className="p-4 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
...
function LedgerTab({ ledger, members }: { ledger: FamilyLedger[]; members: FamilyMember[] }) {
  ...
  if (ledger.length === 0) {
    return <EmptyState icon="🎁" message="부조/선물 기록이 없습니다" sub="기록을 시작해보세요" />;
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
          <p className="text-xs text-blue-600 dark:text-blue-400">받은 총액</p>
          <p className="text-xl font-bold text-blue-700 dark:text-blue-300">{totalReceived.toLocaleString()}원</p>
        </div>
        <div className="p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
          <p className="text-xs text-red-600 dark:text-red-400">보낸 총액</p>
          <p className="text-xl font-bold text-red-700 dark:text-red-300">{totalSent.toLocaleString()}원</p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {ledger.map(l => (
          <div key={l.id} className="flex items-center gap-3 p-3 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
            <span className="text-xl">{l.category === 'gift' ? '🎁' : '💰'}</span>
...
function MemoriesTab({ memories, members }: { memories: FamilyMemory[]; members: FamilyMember[] }) {
  ...
  if (memories.length === 0) {
    return <EmptyState icon="📝" message="기록된 추억이 없습니다" sub="가족의 이야기를 남겨보세요" />;
  }

  return (
    <div className="space-y-3">
      {memories.map(m => (
        <div key={m.id} className="p-4 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
          <div className="flex items-start gap-3">
            <span className="text-2xl mt-0.5">📝</span>
            <div className="flex-1">
              <p className="font-medium text-gray-900 dark:text-white">{m.title}</p>
              {m.content && <p className="text-sm text-gray-500 mt-1 whitespace-pre-wrap">{m.content}</p>}
