...
async function fetchTranscript(videoId: string): Promise<{
  ...
  // Method 0: youtubei.js 라이브러리 (Innertube API 래퍼 - 가장 안정적)
  try {
    ...
    if (segList.length > 0) {
      ...
      for (const seg of segList) {
        ...
        if (renderer) {
          ...
          const endMs = parseInt(renderer.endMs || "0", 10);
          if (text) {
            segments.push({ start: startMs / 1000, duration: (endMs - startMs) / 1000, text });
          }
        }
      }
      if (segments.length > 0) {
        console.log(`youtubei.js: extracted ${segments.length} segments`);
        return {
          ok: true,
          text: segments.map(s => s.text).join(" "),
          segments,
    ...
    console.log("youtubei.js: no segments found in transcript response");
  } catch (e) {
