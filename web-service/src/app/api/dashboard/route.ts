...
export async function GET() {
  try {
    ...
    // 에이전트 HR 데이터 (성과)
    let agentHrData: Record<string, unknown>[] = [];
    try {
    ...
    const services = SERVICES.map((svc) => {
      if (svc.id === "slack-agents") {
        return {
          ...
          health: flyHealth.data || null,
        };
      }
      if (svc.id === "web-service") {
        return { ...svc, status: "running", health: null }; // 이 API가 응답하면 웹서비스는 동작 중
      }
      return { ...svc, status: "unknown", health: null };
    });

    // 슬랙 에이전트 상태 조합
    const slackAgents = SLACK_AGENTS.map((agent) => {
      const hr = agentHrData.find((h: Record<string, unknown>) => h.agent_name === agent.id);
      const taskStats = agentTaskStats[agent.id];
      return {
        ...agent,
        status: flyHealth.ok ? "running" : "unknown",
        tasks_24h: taskStats || { total: 0, completed: 0, failed: 0 },
        hr: hr || null,
      };
    });
    ...
    return NextResponse.json({
      ...
      slackAgents,
      ghAgents,
      recentActions: (actions as unknown[]).slice(0, 10),
