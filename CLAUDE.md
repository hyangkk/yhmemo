...
### 3. QA 대원칙 (필수)
- **모든 배포 전과 후에 반드시 QA 테스트를 수행한다.** 예외 없음.
- **배포 전**: TypeScript 빌드 체크 (`npx tsc --noEmit`), 핵심 로직 검증
- **배포 후**: QA 에이전트가 자동으로 웹 서비스 엔드포인트 헬스체크 실행 (10분 간격 상시 모니터링)
- **배포 실패 또는 서비스 다운 감지 시**: 즉시 슬랙 `ai-agents-general` 채널에 알림
- **수동 QA**: `!qa` 또는 `!테스트` 명령어로 즉시 실행 가능
- QA 에이전트 위치: `slack-agents/agents/qa_agent.py` (Fly.io에서 24/7 가동)
- **"코드상 맞는 것 같다"는 보고 금지** — 배포 후 실제 서비스에서 동작 확인한 결과만 보고
