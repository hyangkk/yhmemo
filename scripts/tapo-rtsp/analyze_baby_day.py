...
def connect_camera():
    ...
    tapo = Tapo(CAMERA_IP, CAMERA_USER, CAMERA_PASS, CAMERA_PASS)
    print("연결 성공!")
    return tapo
...
async def download_samples(tapo, recs, date_str, start_hour, end_hour, quick=False):
    """시간대별 샘플 다운로드"""
            ...
            for seg in all_segments:
                if seg["startTime"] <= target_ts <= seg["endTime"]:
                    sample_points.append({
                        ...
                        "start": target_ts,
                        "end": min(target_ts + 20, seg["endTime"]),  # 20초 샘플
    ...
    for i, sp in enumerate(sample_points):
        ...
        try:
            ...
            if filename and os.path.exists(filename):
                ...
                downloaded.append({"label": label, "hour": sp["hour"], "file": filename})
            else:
                # 디렉토리에서 mp4 파일 찾기
                mp4s = [f for f in os.listdir(out_dir) if f.endswith(".mp4")]
                if mp4s:
                    filename = os.path.join(out_dir, mp4s[0])
                    size_kb = os.path.getsize(filename) / 1024
                    print(f" OK ({size_kb:.0f}KB)")
                    downloaded.append({"label": label, "hour": sp["hour"], "file": filename})
                else:
...
def analyze_motion(video_path):
    ...
    frame_skip = max(1, int(fps))  # 1초마다 샘플링
    prev_gray = None
    scores = []
    frame_idx = 0
...
async def main():
    ...
    # 3. 타임라인 출력
    segments = print_recording_timeline(recs, args.date)

    # 4. 시간대별 샘플 다운로드
    downloaded = await download_samples(tapo, recs, args.date, args.start, args.end, args.quick)
    if not downloaded:
